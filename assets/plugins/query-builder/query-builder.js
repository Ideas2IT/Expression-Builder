...
(function($){
    ...
    var QueryBuilder = function($el, options) {
        ...
        // check filters comment by mahesh for add filter as dynamically
        /* 
    ...
    QueryBuilder.DEFAULTS = {
        ...
        lang: {
            ...
            filter_table_select_placeholder: 'Select table..',
            filter_field_select_placeholder: 'Select field..',
    ...
    /**
     * Update table in combo box
     */
    QueryBuilder.prototype.updateTableCombo = function(options) {
      if(options.mode == "add"){
    	  QueryBuilder.DEFAULTS.tables.push({
    		  "id"    : options.tableName,
    		  "label" : options.tableName
    	  });
    	  $(".rules-list").find("select.table-filter").each(function(){
              $(this).append('<option value="'+ options.tableName +'">'+ options.tableName +'</option>');
      	  });
    	  $(".rules-list").find(".table-filter").selectpicker('refresh');
      }else if(options.mode == "delete"){
    	  var tableNameIndex;
    	  $.each(QueryBuilder.DEFAULTS.tables, function(index, table) {
    		  if(table.id == options.tableName){
    			  tableNameIndex = index;
    		  }
    	  }); 
    	  QueryBuilder.DEFAULTS.tables.splice(tableNameIndex, 1);
    	  $(".rules-list").find("select.table-filter option[value='"+options.tableName+"']").remove();
    	  $(".rules-list").find(".table-filter").selectpicker('refresh');
      }      
    };
    
    /**
     * Update column in combo box
     */
    QueryBuilder.prototype.updateColumnCombo = function(options) {
    	var self = this;
    	$("#"+options.rule_id+"_filter").empty();
    	$(".rules-list").find("#"+options.rule_id+"_filter").append('<option>'+ this.lang.filter_field_select_placeholder +'</option>');
    	$.each(options.columns, function(index, column) {
    		self.getExpressionFiltersData(column, options.tableName);
    		$(".rules-list").find("#"+options.rule_id+"_filter").append('<option value="'+ column.id +'">'+ column.field +'</option>');
    	});
    	$(".rules-list").find("#"+options.rule_id+"_filter").selectpicker('refresh');
    	this.filters = QueryBuilder.DEFAULTS.filters;
    	this.checkFilters();
    };
    

    QueryBuilder.prototype.getExpressionFiltersData = function(column, tableName){
		var localColumnType = "string";
		var filterProperty = {
			'id'    : column.id,
        	'label' : column.field,
        	'table' : tableName
		...
		if(columnType === "boolean"){
			//localColumnType = "boolean";
			filterProperty["input"]  = "radio";
			filterProperty["values"] = {
			    1: 'Yes',
			    0: 'No'
			}
			filterProperty["dataType"] = "boolean";
			filterProperty["operators"] = ['equal'];
		}else if(columnType === "string"){
			localColumnType = "string";
		}else if(columnType == "int"){
			localColumnType = "integer";
		}else if(columnType === "decimal"){
			localColumnType = "double";
		}else if(columnType === "time"){
			localColumnType = "datetime";
			filterProperty["plugin"] = "clockpicker";
			filterProperty["plugin_config"] = {
				    autoclose: true
			}	
		}else if(columnType === "datetime"){
			localColumnType = "datetime";
			filterProperty["plugin"] = "datetimepicker";
		}else if(columnType ===  "date"){
			localColumnType = "datetime";
			filterProperty["plugin"] = "datepicker";
			filterProperty["plugin_config"] = {
				format         : 'yyyy/mm/dd',
			    todayBtn       : 'linked',
			    todayHighlight : true,
		...
		filterProperty["type"] = localColumnType;
    	QueryBuilder.DEFAULTS.filters.push(filterProperty);
    };
    ...
    QueryBuilder.prototype.addRule = function(container) {
        var rule_id = this.nextRuleId(),
            ...
            $tableSelect     = $(this.getTableSelect(rule_id));

        container.append($rule);
        $tableSelect.insertBefore($rule.find('.rule-filter-container'));
    ...
    QueryBuilder.prototype.createRuleInput = function($rule, filter) {
        ...
        for (var i=0; i<operator.accept_values; i++) {
            ...
            if (i > 0) $valueContainer.append(' and ');
...
<dl id="'+ group_id +'" class="rules-group-container" '+ (this.settings.sortable ? 'draggable="true"' : '') +'> \
  <dt class="rules-group-header"> \
    <div class="btn-group pull-right"> \
      <button type="button" data-original-title="Add Rule" data-toggle="tooltip" class="btn btn-xs btn-success" data-add="rule"> \
        <i class="' + this.settings.icons.add_rule + '"></i>  \
      </button> \
      '+ (this.settings.allow_groups ? '<button type="button" data-original-title="Add Group" data-toggle="tooltip" class="btn btn-xs btn-success" data-add="group"> \
        <i class="' + this.settings.icons.add_group + '"></i>  \
      </button>' : '') +' \
      '+ (!main ? '<button type="button" class="btn btn-xs btn-danger" data-original-title="Delete Group" data-toggle="tooltip" data-delete="group"> \
        <i class="' + this.settings.icons.remove_group + '"></i>  \
      </button>' : '') +' \
    </div> \
    <div class="btn-group conditional-btn"> \
  ...
  <dd class=rules-group-body> \
    ...
    QueryBuilder.prototype.getGroupConditions = function(group_id) {
        ...
        var switchBtn = "<div class='switch-toggle switch-2 switch-ios'>";
        for (var i=0, l=this.settings.conditions.length; i<l; i++) {
          ...
          /*  h+= '\
            <label class="btn btn-xs '+ (active?'active':'') +'"> \
              <input type="radio" name="'+ group_id +'_cond" value="'+ cond +'" '+ (active?'checked':'') +'> '+ label +' \
            </label>';
            */
            
            var uniqueId = guid();
        	switchBtn+= '<input id="'+group_id+'_'+uniqueId+'" type="radio" name="'+ group_id +'_cond" value="'+ cond +'" '+ (active?'checked':'') +'>';
        	switchBtn+= '<label class="'+ (active?'active':'') +'" onclick="" for="'+group_id+'_'+uniqueId+'">'+ label +'</label>';
        }
        h+= switchBtn + "<a></a></div>";
        return h;
    };
...
<li id="'+ rule_id +'" class="rule-container" '+ (this.settings.sortable ? 'draggable="true"' : '') +'> \
  <div class="rule-header"> \
    <div class="btn-group pull-right"> \
      <button data-original-title="Delete Rule" data-toggle="tooltip" type="button" class="btn btn-xs btn-danger" data-delete="rule"> \
  ...
  <div class="rule-value-container"></div> \
    ...
    QueryBuilder.prototype.getRuleFilterSelect = function(rule_id) {
        var h = '<select data-live-search="true" class="field-filter selectpicker btn-sm" id="'+ rule_id +'_filter" name="'+ rule_id +'_filter">';
        h+= '<option value="-1">'+ this.lang.filter_field_select_placeholder +'</option>';
    ...
    QueryBuilder.prototype.getTableSelect = function(rule_id) {
        var h = '<select data-live-search="true" class="table-filter selectpicker btn-sm" onChange = "loadFilterCombo(this, \''+rule_id+'\');" name="table-filter">';
        h+= '<option value="-1">'+ this.lang.filter_table_select_placeholder +'</option>';

        $.each(this.tables, function(i, filter) {
            h+= '<option value="'+ filter.id +'">'+ filter.label +'</option>';
        });
    ...
    QueryBuilder.prototype.getRuleOperatorSelect = function(rule_id, operators) {
        var h = '<select data-live-search="true" class="operator-combo selectpicker btn-sm" name="'+ rule_id +'_operator">';
        ...
        switch (filter.input) {
            case 'radio':
                ...
                if(filter.dataType == "boolean"){
                     var switchBtn = "<div class='switch-toggle switch-2 switch-ios boolean-switch'>";
                     iterateOptions(filter.values, function(key, val) {
                     	var uniqueId = guid();
                     	switchBtn+= '<input checked="" id="'+name+'_'+uniqueId+'" type="radio" name="'+ name +'" value="'+ key +'">';
                     	switchBtn+= '<label onclick="" for="'+name+'_'+uniqueId+'">'+ val +'</label>';
                     });
                     switchBtn+= "<a></a></div>";
                     h+=switchBtn;
                }else{
            ...
            case 'select':
                h+= '<select class="rule-input-select" name="'+ name +'"'+ (filter.multiple ? ' multiple' : '') +'>';
                iterateOptions(filter.values, function(key, val) {
            ...
            case 'text': default:
                switch (filter.internalType) {
                    case 'number':
                        h+= '<input class="rule-input-number" type="number" name="'+ name +'"';
                    ...
                    case 'datetime': case 'text': default:
                        h+= '<input class="rule-input-date" type="text" name="'+ name +'"';
                        if (filter.placeholder) h+= ' placeholder="'+ filter.placeholder +'"';
                        h+= '>';
                }
        }
